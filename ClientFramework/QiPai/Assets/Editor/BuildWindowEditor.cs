...
public class BuildWindowEditor : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        GUI.BeginGroup(new Rect(0, 160, 310, 200));
        {
            ...
            if (GUI.Button(new Rect(270, 0, 40, 20), "保存"))
            {
                SaveInfor();
            }
    ...
    public static void ReadGameDataInfo()
    {
        ...
        else if (GameBuildSettingSelect.systemPlatform == 系统平台.Windows)
        {
            fix = "exe";
            GameBuildSettingSelect.platformSelectOutSystemFoloder = GameBuildSettingSelect.platformSelectFoloder + "/WindowsOut";
        }
		else if (GameBuildSettingSelect.systemPlatform == 系统平台.WebGL)
		{
			fix = "gl";
			GameBuildSettingSelect.platformSelectOutSystemFoloder = GameBuildSettingSelect.platformSelectFoloder + "/WebGLOut";
		}
        ...
        if (!Directory.Exists(GameBuildSettingSelect.platformSelectFoloder + "/WindowsOut"))
        {
            Directory.CreateDirectory(GameBuildSettingSelect.platformSelectFoloder + "/WindowsOut");
        }

		if (!Directory.Exists(GameBuildSettingSelect.platformSelectFoloder + "/WebGLOut"))
		{
			Directory.CreateDirectory(GameBuildSettingSelect.platformSelectFoloder + "/WebGLOut");
		}

		if (!Directory.Exists(GameBuildSettingSelect.platformSelectOutSystemFoloder + "/Binary"))
        {
            Directory.CreateDirectory(GameBuildSettingSelect.platformSelectOutSystemFoloder + "/Binary");
        }
        if (!File.Exists(GameBuildSettingSelect.platformSelectOutSystemFoloder + "/Binary/install." + fix))
        {
            File.WriteAllBytes(GameBuildSettingSelect.platformSelectOutSystemFoloder + "/Binary/install." + fix, Encoding.Default.GetBytes("00000"));
        }

        GameBuildSettingSelect.serverAddrPath = GameBuildSettingSelect.platformSelectOutSystemFoloder + "/ServerAddress.txt";
        GameBuildSettingSelect.versionPath = GameBuildSettingSelect.platformSelectOutSystemFoloder + "/Version.txt";
        GameBuildSettingSelect.resourceVersionPath = GameBuildSettingSelect.platformSelectOutSystemFoloder + "/ResourceVersion.txt";

        if (!File.Exists(GameBuildSettingSelect.serverAddrPath))
        {
            File.WriteAllBytes(GameBuildSettingSelect.serverAddrPath, Encoding.Default.GetBytes(""));
        }

        if (!File.Exists(GameBuildSettingSelect.versionPath))
        {
            File.WriteAllBytes(GameBuildSettingSelect.versionPath, Encoding.Default.GetBytes("0.0.0"));
        }

        byte[] versionBuf = File.ReadAllBytes(GameBuildSettingSelect.versionPath);
        GameBuildSettingSelect.currentVersion = System.Text.Encoding.Default.GetString(versionBuf);
        ...
        GameBuildSettingSelect.assetsServerAddr = System.Text.Encoding.Default.GetString(CompressEncryption.UnEncryption(serAddrBuf));
    }
    ...
    {
        File.WriteAllBytes(GameBuildSettingSelect.serverAddrPath, CompressEncryption.Encryption(Encoding.Default.GetBytes(GameBuildSettingSelect.assetsServerAddr)));
    ...
    {
        EditorUtility.DisplayProgressBar("保存设置", "正在保存打包设置", 1.0f);
        SaveInfor();
        EditorUtility.ClearProgressBar();

        if(GameBuildSettingSelect.onlyPatch)
        {
            MakeSpawnAssetTool.MakeUpVersionFile();
        }
        ...
        if (GameBuildSettingSelect.needUpAsset)
        {
            ...
            {
                EditorUtility.DisplayProgressBar("删除资源目录", "删除" + Application.streamingAssetsPath, 0.9f);
                Directory.Delete(Application.streamingAssetsPath, true);
                EditorUtility.ClearProgressBar();
            }

            EditorUtility.DisplayProgressBar("拷贝资源到发布工程", "准备拷贝", 0.9f);
            EditorUtility.ClearProgressBar();

            MakeHotFixBuild.CopyDirectory(GameBuildSettingSelect.platformSelectOutSystemFoloder, Application.dataPath);
            string[] dirArray = GameBuildSettingSelect.platformSelectOutSystemFoloder.Split('/');
            Directory.Move(Application.dataPath + "/" + dirArray[dirArray.Length-1], Application.streamingAssetsPath);

            if (Directory.Exists(Application.streamingAssetsPath + "/Binary"))
            {
                Directory.Delete(Application.streamingAssetsPath + "/Binary", true);
            }
        }
    ...
    public static void Build()
    {
        ...
        if (GameBuildSettingSelect.systemPlatform == 系统平台.Android)
        {
            BuildPipeline.BuildPlayer(names.ToArray(), GameBuildSettingSelect.platformSelectOutSystemFoloder + "/Binary/install" + ".apk", BuildTarget.Android, BuildOptions.None);
        }
        else if(GameBuildSettingSelect.systemPlatform == 系统平台.IOS)
        {
            if (GameBuildSettingSelect.scriptingBackend == ScriptingImplementation.IL2CPP)
            {
                BuildPipeline.BuildPlayer(names.ToArray(), GameBuildSettingSelect.platformSelectOutSystemFoloder + "/Binary/install" + PlayerSettings.productName + GameBuildSettingSelect.outVersion, BuildTarget.iOS, BuildOptions.None);
            }
            else
            {
                BuildPipeline.BuildPlayer(names.ToArray(), GameBuildSettingSelect.platformSelectOutSystemFoloder + "/Binary/install" + PlayerSettings.productName + GameBuildSettingSelect.outVersion, BuildTarget.iOS, BuildOptions.None);
        ...
        {
            BuildPipeline.BuildPlayer(names.ToArray(), GameBuildSettingSelect.platformSelectOutSystemFoloder + "/Binary/install" +  ".exe", BuildTarget.StandaloneWindows, BuildOptions.None);
        }
		else if (GameBuildSettingSelect.systemPlatform == 系统平台.WebGL)
		{
			BuildPipeline.BuildPlayer(names.ToArray(), GameBuildSettingSelect.platformSelectOutSystemFoloder + "/Binary/install" + PlayerSettings.productName + GameBuildSettingSelect.outVersion, BuildTarget.WebGL, BuildOptions.None);
		}
	}
...
/// 更新生成工具
/// </summary>
public class MakeSpawnAssetTool
{
    public static void MakeUpVersionFile()
    {
        string binaryFix = "exe";
        string systemOut = "/WindowsAssets";
        if(GameBuildSettingSelect.systemPlatform == 系统平台.Android)
        {
            systemOut = "/AndroidAssets";
            binaryFix = "apk";
        }
        else if(GameBuildSettingSelect.systemPlatform == 系统平台.IOS)
        {
		...
		{
			systemOut = "/WebGLAssets";
			binaryFix = "gl";
		}

		MakeHotFixBuild.SpawnMD5UpVersionFile(
            GameBuildSettingSelect.versionPath,
            GameBuildSettingSelect.resourceVersionPath,
            GameBuildSettingSelect.assetsFoloder,
            GameBuildSettingSelect.platformSelectOutSystemFoloder,
            GameBuildSettingSelect.platformSelect.ToString(),
            systemOut,
            binaryFix,
            true, 
            true);
