...
Shader "Custom/MobileBloom" {
	...
	fixed4 frag_round(float2 pos, float4 color) : SV_Target
	{
    ...
    {  
        fixed4 ori = tex2D(_MainTex, i.uv2);
	...
	fixed4 frag_hsv(v2f_hsv i) : SV_Target
	{
	...
	fixed4 frag_hsv_2(v2f_hsv i) : SV_Target
	{
		half4 col = tex2D(_MainTex, i.texcoord);
