...
public class Book : MonoBehaviour {
    ...
    public Sprite[] bookPages=new Sprite[2];
    ...
    //current flip mode
    FlipMode mode;
    private Transform _rightHotSpot;

    private Transform RightHotSpot {
        get { return _rightHotSpot ?? (_rightHotSpot = transform.Find("RightHotSpot")); }
    }

    private const float FLIP_TIME = 0.2f;

    public void Init(Canvas canvas) {
        _canvas = canvas;
    }

    public void SetCardImage(Sprite back, Sprite card) {
        bookPages[0] = back;
        bookPages[1] = card;
    }
    ...
    public void StopAndOpen() {
        if (currentPage == 0) {
            DragRightPageToPoint(RightHotSpot.localPosition);
    ...
    void Update()
    {
        ...
        //Vector3 pos = Right.transform.localPosition;
        //pos.x = RightNext.transform.localPosition.x;
        //Right.transform.localPosition = pos;

        //pos = Left.transform.localPosition;
        //pos.x = RightNext.transform.localPosition.x;
    ...
    public void DragRightPageToPoint(Vector3 point)
    {
        ...
        Left.enabled = (currentPage < bookPages.Length);
        Left.transform.SetAsFirstSibling();
        ...
        Right.enabled = (currentPage < bookPages.Length - 1);

        RightNext.sprite = (currentPage < bookPages.Length - 2) ? bookPages[currentPage + 2] : background;
        RightNext.enabled = (currentPage < bookPages.Length - 2);
    ...
    public void DragLeftPageToPoint(Vector3 point)
    {
        ...
        Left.enabled = (currentPage < bookPages.Length - 1);

        LeftNext.sprite = (currentPage < bookPages.Length-2) ? bookPages[currentPage +2] : background;
        LeftNext.enabled = (currentPage < bookPages.Length - 2);
    ...
    void UpdateSprites()
    {
        ...
        LeftNext.enabled = (currentPage > 0 && currentPage <= bookPages.Length);
        RightNext.enabled = (currentPage >= 0 && currentPage < bookPages.Length);
    }
    private void TweenForward() {
        ...
        if(mode== FlipMode.RightToLeft)
            currentCoroutine = StartCoroutine(TweenTo(ebl, FLIP_TIME, () => { Flip(); }));
        else
            currentCoroutine = StartCoroutine(TweenTo(ebr, FLIP_TIME, () => { Flip(); }));
    }
    ...
    public void TweenBack()
    {
        if (mode == FlipMode.RightToLeft)
        {
            currentCoroutine = StartCoroutine(TweenTo(ebr, FLIP_TIME,
                () =>
                {
                    UpdateSprites();
        ...
        {
            currentCoroutine = StartCoroutine(TweenTo(ebl, FLIP_TIME,
                () =>
                {
                    UpdateSprites();
