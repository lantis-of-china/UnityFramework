...
public class MakeHotFixBuild
{
    ...
    /// <returns></returns>
    public static string SpawnMD5UpVersionFile(string versionPath, string resourceVersionPath, string assetsRootPath, string hotfoxPath, string platformPath, string systemFloder, bool compress, bool copyAsset)
    {
        string parkPath = "";
        string path = "";
        string clientInfor = "";
        ...
        parkPath = exportPath + platformPath + "/Binary/install." + PlatformExe();
        ...
        string versionStr = Encoding.UTF8.GetString(byteBuf);
        if (versionStr == "")
        {
            versionStr = "0.0.1";
        }
        ...
        {
            // 首先查找最后版本中的文件 、、
            fileListName = GetFileListPath(assetsRootPath + systemFloder, "*.*");
        }
        else
        {
            fileListName = GetFileListPath(Application.streamingAssetsPath + systemFloder, "*.*");
            assetsRootPath = Application.streamingAssetsPath;
        }
        ...
        for (int loop = 0; loop < fileListName.Count; loop++)
        {
            ...
            filePath = filePath.Replace(assetsRootPath + "/", "");
        ...
        byte[] md5IfoBuf = Encoding.UTF8.GetBytes(clientInfor);
        if (compress)
        {
            md5IfoBuf = CompressEncryption.Encryption(md5IfoBuf);
        }

        File.WriteAllBytes(assetsRootPath + "/ResourceVersion.txt", md5IfoBuf);
        File.WriteAllBytes(hotfoxPath + platformPath + "/ResourceVersion.txt", md5IfoBuf);
        fileListName.Clear();

        if (copyAsset)
        {
            CopyDirectory(assetsRootPath + systemFloder, hotfoxPath + platformPath);
        }
    ...
    public static Dictionary<string, ResourceTemplate> GetResourceMd5(string md5Text)
    {
        ...
        string[][] TableString = CsvAnalysic.SplitCSV(md5Text);

        Dictionary<string, ResourceTemplate> results = new Dictionary<string, ResourceTemplate>();

        for (int loop = 1; loop < TableString.Length; loop++)
        {
            int fieldIndex = 0;
    ...
    public static bool IsCanWrtileFile(FileInfo fi)
    {
        if (fi.Exists)
        {
            if (fi.Extension.ToLower() == "meta" || fi.Name.Contains(".DS_Store"))
            {
                return false;
            }
